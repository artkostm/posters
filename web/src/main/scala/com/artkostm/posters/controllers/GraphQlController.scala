...
class GraphQlController @Inject()(repository: PostgresPostersRepository, system: ActorSystem) extends Controller {
  ...
  post("/test") { request: GraphQlRequest =>
    QueryParser.parse(request.query) match {
      case Success(queryAst) =>
        Executor.execute(TestSchema.instance, queryAst, new SimpleRepo, operationName = request.operationName)
