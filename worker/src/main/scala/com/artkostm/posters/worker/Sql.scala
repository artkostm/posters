...
object Sql extends IOApp {
  ...
  override def run(args: List[String]): IO[ExitCode] = {
    val transactor = for {
      ...
      xa <- HikariTransactor.newHikariTransactor[IO](driverClassName = "org.postgresql.Driver",
    ...
    transactor.use { xa =>
      val y = xa.yolo
      import y._
      sc.transact(xa).map(println).as(ExitCode.Success)
    }
  }
}
